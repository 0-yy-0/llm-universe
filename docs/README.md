...
文档站完整课程入口：[C7 高级 RAG 技巧](https://github.com/datawhalechina/llm-universe/blob/HEAD/notebook/C7%20高级%20RAG%20技巧/README.md)。C7 不是基础 RAG 的替代品，开始前请先完成下面的补课；每个入口都对应现有 C2—C5 内容。
...
| 会用基本检索指标复查改动 | [C7 最小指标预习](https://github.com/datawhalechina/llm-universe/blob/HEAD/notebook/C7%20高级%20RAG%20技巧/README.md#最小指标预习)、[C5：评估应用](C5/1.%20如何评估%20LLM%20应用.md) | 能在固定问题和相关证据上计算 Recall@k、首条相关排名与 MRR，并保留全部逐题结果 |
...
4. 检索阶段：提取筛选条件、过滤无权资料、改写问题、判断是否继续检索，以及用 PRCA/REPLUG 对齐检索与生成。
...
C7 的向量微调实验使用 163 条合成 query→evidence pair（101 train、28 dev、34 frozen test），各 split 的 PDF 页面互不重叠。问法由 `glm-4-flash` 生成、初审并通过同一模型的另一次独立请求复核，记录均为 `human_verified=false`；完整数据审核、切分和实验边界见[C7 第 2 章](https://github.com/datawhalechina/llm-universe/blob/HEAD/notebook/C7%20高级%20RAG%20技巧/2.%20数据处理/README.md)。冻结测试集上的 9 条改善、3 条退化、22 条不变只适用于该有限合成分布。
