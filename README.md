...
C7 的向量微调实验使用 163 条合成 query→evidence pair（101 train、28 dev、34 frozen test），各 split 的 PDF 页面互不重叠。问法由 `glm-4-flash` 生成、初审并通过同一模型的另一次独立请求复核，记录均为 `human_verified=false`；完整数据审核、切分和实验边界见[C7 第 2 章](notebook/C7%20高级%20RAG%20技巧/2.%20数据处理/README.md)。冻结测试集上的 9 条改善、3 条退化、22 条不变只适用于该有限合成分布。
