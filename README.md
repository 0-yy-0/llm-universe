...
目前，第一部分已经完稿，欢迎大家阅读学习；第二部分整理为进阶 RAG 技巧，第三部分仍在创作中。
...
这一部分围绕“为什么答错、怎样定位、怎样改进”展开。请从教程入口开始，再根据遇到的问题选择内容：

- [高级 RAG 教程](notebook/C7%20高级%20RAG%20技巧/README.md)

当前教程按七章组织：

1. 背景：为什么基础 RAG 还会答错，以及怎样区分资料、检索和回答问题。
2. 数据处理：读取和清理资料、比较分块方式、选择向量模型，以及了解何时需要微调。
3. 索引阶段：保存资料属性，为片段补充背景和检索说法，并结合关键词和向量检索。
