...
| 实验 | 固定的范围 | 能回答的问题 |
|---|---|---|
| [C7 第 3 章索引增强比较](../3.%20索引阶段/比较索引增强方法.ipynb) | 3 道适配题、9 页候选语料、BM25、`top_k=3`、同一检索字段预算 | 方法是否适合它针对的问法（method-fit） |
...
C7 第 3 章保存的局部适配结果中，可靠章节标题（CCH）让“机器学习算法之间有没有绝对更好”这道题的必要页首条排名由 3 提到 2；Document Augmentation 让“集成多个弱学习器”这道题由 2 提到 1。这是检索排名改善，回答质量仍需另查。

C7 第 3 章的 3 题包含于本章的 6 题，二者并非独立的泛化验证；候选语料、证据粒度、`top_k` 和预算也不同。各专题 Notebook 同样使用各自的输入范围，其改善、持平、退化与不适用结果应留在对应实验中，不能合并成一张跨方法排行榜。
...
微调数据包的 34 条 frozen test 问法来自 17 个 PDF 页面上的固定 token chunks，每个 chunk 最多 1 题。问法由 `glm-4-flash` 生成和初审，再由同一 `glm-4-flash` 发起另一次独立语义审核请求；保留记录均为 `human_verified=false`。这是有限的合成问法分布，与本章 6 道回归题用途不同，均不能外推为自然用户集覆盖，也不能把模型审核称为人工核验。
...
## 实践任务：建立一个最小 regression guard

从自己的资料中选 4 个问题：2 个当前答错目标、2 个原本能答的问题。固定资料版本、qrels/必要证据、检索器、`top_k` 和上下文预算，先跑基线，再只改一个 C7 方法；逐题保存 Recall@k、MRR 或首条相关排名、证据覆盖、上下文长度与回答支持情况。

### 自检标准

- 4 个问题的 query、必要证据、候选池、预算和方法版本都可复现，不能改完方法后再挑题。
- 目标题与原本正常题分开计数，并保存全部 `improved`、`unchanged`、`degraded` 或 `tradeoff` 逐题结果；若出现不变或退化，不得删题或用平均分掩盖，但不要求实验刻意产生这些结果。
- 检索指标和回答依据分开判定：命中页不等于答案正确，回答结论必须回到原文；资料不足题应明确拒答。
